...
- Миграциите се прилагат само от `supabase/migrations/` (не от `drizzle/`). Последната е `20260924134042_signature_viewed_reminders`.
...
- Имейлите, които приложението праща: линк към офертата, код за потвърждение, разписка с PDF, „обновена оферта“ с разликите, напомняния (3 дни без решение и 2 дни преди края на срока).
