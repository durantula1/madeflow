...
export default async function ChangesPage({ searchParams }: PageProps<"/app/changes">) {
  ...
  const changes = rows.slice(0, 20);
  return (
    <>
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-primary">
            {workspacePageCopy.changes.eyebrow}
          </p>
          <h1 className="mt-1 text-3xl font-semibold tracking-tight">
            {workspacePageCopy.changes.title}
          </h1>
          <p className="mt-2 text-muted-foreground">
            {workspacePageCopy.changes.description}
          </p>
        </div>
        <Link
      ...
      <Card className="mt-7">
        <CardHeader className="border-b">
          <CardTitle>Всички промени</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {changes.length ? (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {change.projectName} · версия {change.revisionNumber}
                    </p>
                  </div>
            ...
            <div className="py-14 text-center">
              <p className="font-medium">Няма промени</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Промяна се създава към оферта, която клиентът вече е одобрил.
              </p>
            </div>
          )}
        </CardContent>
      </Card>
      <ListPagination path="/app/changes" params={{ q: query, status, projectId }} page={page} hasNext={rows.length > 20} />
    </>
  );
}
