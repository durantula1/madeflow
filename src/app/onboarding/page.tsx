...
export default async function OnboardingPage() {
  ...
  return <main className="min-h-screen px-5 py-6"><div className="mx-auto max-w-5xl"><Wordmark /><div className="mt-12 grid overflow-hidden rounded-[2rem] border bg-card shadow-xl shadow-stone-900/5 lg:grid-cols-2"><section className="p-7 sm:p-10"><p className="text-sm font-semibold text-primary">Последна стъпка</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Настрой работното пространство</h1><p className="mt-3 text-muted-foreground">Тук ще живеят клиентите, поръчките и историята на екипа.</p><OnboardingForm defaultName={name} /></section><aside className="bg-[#24261f] p-8 text-stone-100 sm:p-10"><Building2 className="size-10 text-[#8ac99a]" /><h2 className="mt-8 text-2xl font-semibold">Какво получаваш веднага</h2><ul className="mt-6 space-y-4 text-sm text-stone-300">{["Шаблон за мебели и кухни","Последователни номера на поръчките","Защитени клиентски връзки","Версии и неизменима следа на одобрението"].map(item => <li key={item} className="flex gap-3"><Check className="mt-0.5 size-4 shrink-0 text-[#8ac99a]" />{item}</li>)}</ul></aside></div></div></main>;
}
