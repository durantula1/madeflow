...
export function ProjectOverview({ state, portalPublicId, showPayments = true, section = "all" }: { state: State; portalPublicId?: string; showPayments?: boolean; section?: "all" | "summary" | "work" | "payments" }) {
  ...
  return <div className="space-y-5">
    {(section === "all" || section === "summary") ? <section className="rounded-2xl border bg-card p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl bg-muted p-4"><p className="text-xs text-muted-foreground">Крайна цена на обекта</p><p className="mt-1 text-xl font-semibold">{state.offer ? formatCents(state.contractMinor, state.currency) : "Очаква одобрена оферта"}</p></div>
        <div className="rounded-xl bg-muted p-4"><p className="text-xs text-muted-foreground">Получено</p><p className="mt-1 text-xl font-semibold">{formatCents(state.paidMinor, state.currency)}</p></div>
        <div className="rounded-xl bg-muted p-4"><p className="text-xs text-muted-foreground">Оставащо</p><p className="mt-1 text-xl font-semibold">{state.offer ? formatCents(state.remainingMinor, state.currency) : "—"}</p></div>
        <div className="rounded-xl bg-muted p-4"><p className="text-xs text-muted-foreground">Договорен краен срок</p><p className="mt-1 text-xl font-semibold">{state.deadline ?? "Очаква одобрение"}</p></div>
      </div>
