    ...
    <div ref={ref} className="mx-auto w-full max-w-[20.625rem]">
      <div className="rounded-[2.375rem] bg-[#102b38] p-2.5 shadow-[0_30px_70px_rgba(16,43,56,.3)]">
        <div className="relative min-h-[29.375rem] overflow-hidden rounded-[1.875rem] bg-[#fffaf0] px-5 pb-6 pt-8">
          ...
          <div className="flex items-center justify-between font-mono demo-text-8 tracking-[0.12em] text-[#6c858d]">
          ...
          <AnimatePresence mode="wait">
            {step <= 1 && (
              <motion.div key="decide" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="mt-5">
                <p className="font-mono demo-text-8 tracking-[0.12em] text-[#6c858d]">ИМЕ И ФАМИЛИЯ</p>
            ...
            {(step === 2 || step === 3) && (
              <motion.div key="code" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="mt-5">
                ...
                <p className="mt-3 font-mono demo-text-8 tracking-[0.1em] text-[#6c858d]">ВАЛИДЕН 10 МИН · ДО 5 ОПИТА</p>
              </motion.div>
            )}
            {step >= 4 && (
