...
export function RevisionForm({ initial, withdrawsRevision, catalog = [] }: { initial: Initial; withdrawsRevision?: number; catalog?: CatalogPick[] }) {
  ...
  return <form action={action} className="space-y-5 rounded-2xl border bg-card p-5">
    ...
    {initial.documentKind === "offer" ? <DiscountField defaultType={initial.discountType ?? ""} defaultValue={initial.discountValue ? String(Number(initial.discountValue)) : ""} /> : null}
    ...
    <Field><FieldLabel htmlFor="revision-client-note">Бележка към клиента</FieldLabel><Textarea id="revision-client-note" name="clientNote" defaultValue={initial.clientNote ?? ""} className="min-h-20" /></Field>
    <p className="text-xs text-muted-foreground">Бележки само за екипа се пишат в таб „Бележки“.</p>
