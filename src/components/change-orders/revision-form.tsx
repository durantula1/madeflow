...
export function RevisionForm({ initial }: { initial: Initial }) {
  ...
  return <form action={action} className="space-y-5 rounded-2xl border bg-card p-5">
    ...
    {initial.documentKind === "offer" ? <><input type="hidden" name="changeKind" value="addition" /><input type="hidden" name="subtotal" value="0" /><input type="hidden" name="scheduleImpactType" value="none" /><Field><FieldLabel htmlFor="revision-deadline">Договорен краен срок</FieldLabel><Input id="revision-deadline" type="date" name="agreedDeadline" required defaultValue={initial.agreedDeadline ?? ""} className="h-10" /></Field></> : <div className="grid gap-3 sm:grid-cols-2"><Field><FieldLabel>Отражение върху срока</FieldLabel><input type="hidden" name="scheduleImpactType" value={scheduleType} /><Select aria-label="Отражение върху срока" selectedKey={scheduleType} onSelectionChange={(key) => setScheduleType(String(key))}><SelectTrigger className="h-10"><SelectValue /></SelectTrigger><SelectContent><SelectItem id="none">Без промяна</SelectItem><SelectItem id="days">Нов краен срок</SelectItem></SelectContent></Select></Field>{scheduleType === "days" ? <Field><FieldLabel htmlFor="revision-new-deadline">Нов договорен краен срок</FieldLabel><Input id="revision-new-deadline" name="agreedDeadline" type="date" defaultValue={initial.agreedDeadline ?? ""} required className="h-10" /></Field> : <input type="hidden" name="agreedDeadline" value="" />}</div>}
    <input type="hidden" name="taxRate" value={initial.taxRate} />
    ...
    <Button type="submit" isDisabled={pending}>{pending ? "Запазване…" : "Създай нова версия"}</Button>
  </form>;
}
